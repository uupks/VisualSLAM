...
int main(int argc, char **argv) {
    ...
    cv::Mat desc, desc2;
    auto orb = cv::ORB::create(500, 1.2, 8, 31, 0, 2, 0, 31, 40);
    orb->detectAndCompute(first_image, cv::Mat(), keypoints, desc);
    orb->detectAndCompute(second_image, cv::Mat(), keypoints2, desc2);
    ...
    for (size_t i = 0; i < desc.rows; i++) {
        if (matches[i].distance <= std::max(3 * min_dist, 50.0)) {
    ...
    // plot the matches
    cv::drawMatches(first_image, keypoints, second_image, keypoints2, good_matches, image_show);
    cv::imshow("matches", image_show);
    cv::imwrite("matches.png", image_show);
    cv::waitKey(0);
#else
    int fast_half_patch_size_ = 16;
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
    ...
    for (auto &kp : keypoints) {
        ...
        const uchar* const center = &image.at<uchar>(cvRound(kp.pt.y), cvRound(kp.pt.x));
        ...
        kp.angle = r2a(std::atan2(m_01, m_10));
        // END YOUR CODE HERE
    }
    return;
}
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        double a = a2r(kp.angle);
        double cosa = cos(a);
        double sina = sin(a);

        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            // printf("[%d, %d, %d, %d]\n", ORB_pattern[i * 4 + 0], ORB_pattern[i * 4 + 1], ORB_pattern[i * 4 + 2], ORB_pattern[i * 4 + 3]);
            cv::Point2i p(kp.pt.x + ORB_pattern[i * 4 + 0], kp.pt.y + ORB_pattern[i * 4 + 1]);
            cv::Point2i q(kp.pt.x + ORB_pattern[i * 4 + 2], kp.pt.y + ORB_pattern[i * 4 + 3]);

            int up_ = cosa * p.x - sina * p.y;
            int vp_ = sina * p.x + cosa * p.y;

            int uq_ = cosa * q.x - sina * q.y;
            int vq_ = sina * q.x + cosa * q.y;
            // printf("p : [%d, %d] -> [%d, %d]\n", p.x, p.y, up_, vp_);
            // printf("q : [%d, %d] -> [%d, %d]\n", q.x, q.y, uq_, vq_);
            // if (up_ < 0 || vp_ < 0 || vp_ > image.rows || up_ > image.cols ||
            //         uq_ < 0 || vq_ < 0 || vq_ > image.rows || uq_ > image.cols) {
                    
            //     // if kp goes outside, set d.clear()
            //     d.clear();
            //     break;
            // } else {
                if (image.at<uchar>(vp_, up_) > image.at<uchar>(vq_, uq_)) {
                    d[i] = false;
                } else {
                    d[i] = true;
                }
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for (size_t i = 0; i < desc1.size(); i++) {
        ...
        for (size_t j = 0; j < desc2.size(); j++) {
            ...
            for (size_t n = 0; n < desc1[i].size(); n++) {
                if (desc1[i][n] != desc2[j][n]) {
                    hamm_dis++;
                }
            }
            if (hamm_dis < d_max && hamm_dis > max_hamm) {
                max_hamm = hamm_dis;
                max_index2 = j;
            }
        }
        std::cout<<"Max Hamm : "<<max_hamm<<std::endl;
        if (max_hamm != 0) {
            matches.push_back({i, max_index2, max_hamm});
        }
    }
